...
public class SkipOnEqualsRegister_5XY0_should_
{
    [Fact]
    public void increment_pc_twice_when_vx_equals_vy()
    {
        var opcode = OpcodeGenerator.Create("5XY0");
        ...
        new SkipOnEqualsRegister_5XY0(opcode).Execute(vm);
        
        Assert.Equal(startingPc + 4, vm.PC);
    }
        
    [Fact]
    public void increment_pc_once_when_vx_not_equal_to_vy()
    {
        var opcode = OpcodeGenerator.Create("5XY0");
        var opParser = OpcodeParser.From(opcode);
